...
function [filtro] = pasaBandaKaiser(f_M, f_p1, f_p2, f_s1, f_s2, f_c1, f_c2, graficas)
...
//    wK = (1/besseli(0,Beta))*besseli(0,Beta*sqrt( 1-(1-(2*v_nK)/NK).^2) );
//    
//    v_hn_idK = (2*phi_B)*sinc(%pi*(2*phi_B)*(v_nK-NK/2)).*(2*cos(2*%pi*phi_0*(v_nK-NK/2)));
//
//    v_hnK = v_hn_idK.*wK;
//
...
//            xtitle('(ORIGINAL) Módulo de la transferencia del filtro pasa banda');
//    
